...
function to_utf8(text) {
	...
	for (let i = 0, n = text.length; i < n; ++i) {
		...
		if (c < 0x80) {
			...
			const cp
				= 0x10000
					+ (((c & 0x3FF) << 10) | (text.charCodeAt(++i) & 0x3FF));
			bytes.push(
				0xF0 | ((cp >> 18) & 0x7),
				0x80 | ((cp >> 12) & 0x3F),
				0x80 | ((cp >> 6) & 0x3F),
...
/**
 *
 * @param {Uint8Array | string} buffer - byte array or string
 * @param {number} [seed] - optional seed (32-bit unsigned)
 * @returns {number}
 */
export function xxhash32(buffer, seed = 0) {
	buffer = typeof buffer === "string" ? to_utf8(buffer) : buffer;
	const b = buffer;
	...
	if (b.length >= 16) {
        ...
        */
		...
		for (offset = 0; (offset & 0xFFFFFFF0) <= limit; offset += 4) {
			const i = offset;
			const lane_n0 = b[i + 0] + (b[i + 1] << 8);
			const lane_n1 = b[i + 2] + (b[i + 3] << 8);
			const lane_np = lane_n0 * PRIME32_2 + ((lane_n1 * PRIME32_2) << 16);
			let acc_lane = (acc_n[lane] + lane_np) & 0xFFFFFFFF;
			acc_lane = (acc_lane << 13) | (acc_lane >>> 19);
			const acc0 = acc_lane & 0xFFFF;
			const acc1 = acc_lane >>> 16;
			acc_n[lane]
        ...
        */
		acc
			= (((acc_n[0] << 1) | (acc_n[0] >>> 31))
				+ ((acc_n[1] << 7) | (acc_n[1] >>> 25))
				+ ((acc_n[2] << 12) | (acc_n[2] >>> 20))
				+ ((acc_n[3] << 18) | (acc_n[3] >>> 14)))
			& 0xFFFFFFFF;
	}
    ...
    */
	...
	for (; offset <= limit; offset += 4) {
		...
		const lane_n0 = b[i + 0] + (b[i + 1] << 8);
		const lane_n1 = b[i + 2] + (b[i + 3] << 8);
		const lane_p = lane_n0 * PRIME32_3 + ((lane_n1 * PRIME32_3) << 16);
		acc = (acc + lane_p) & 0xFFFFFFFF;
		acc = (acc << 17) | (acc >>> 15);
		acc
